"""My plan to automate the testing is to have an expected tex output body for
each test.

The main test script will have a list of input
...
def test_one_file(actual_tex_name, expected_tex_name,
                  cut_header=True):
    """Load actual_tex_name and extract its body, i.e.
    ...
    if cut_header:
        ...
        assert len(
            inds2) == 1, 'Did not find exactly one instance of \\end{document}'
        stopind = inds2[0]
...
class tester(object):
    ...
    """A class to try and make it easy to run different rst2beamer tests.

    For now, the primary question is how to handle cases that
    ...
    def run_test(self):
        if os.path.exists(self.tex_name):
            os.remove(
                self.tex_name)  # if the tex file is left laying around and
